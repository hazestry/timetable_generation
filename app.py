...
with st.sidebar:
    ...
    use_ml = st.toggle(
        ...
        help="С ML: генератор ранжирует размещения по реалистичности от классификатора и использует предсказания регрессии для подбора размера аудитории. "
             "Без ML: бейзлайн со случайным порядком слотов."
    )

    seed = st.number_input(
        "Random seed",
        min_value=0, max_value=9999, value=42, step=1,
        help="для воспроизводимости"
    )
...
with tab1:
    ...
    st.download_button(
        "скачать выбранные записи в CSV",
...
with tab3:
    ...
    ax2.set_title('сколько курсов каждого предмета попало в каждый корпус')
